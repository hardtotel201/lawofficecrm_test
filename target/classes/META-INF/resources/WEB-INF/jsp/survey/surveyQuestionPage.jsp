		...
		<form class="form-horizontal" role="form" id="formQuery" name="formQuery" >
			<div class="form-group">
				...
				<label class="font-title col-sm-1 control-label no-padding-right"
				       for="querySurveyTitle">调研题目</label>
				<div class="col-sm-2">
					<input type="text" id="querySurveyTitle" name="querySurveyTitle" class="input-text col-xs-12" />
					<!-- 隐藏HTML元素,作为分页查询条件-->
					<input type="hidden" id="querySurveyId" value="" />
				</div>
				 <label class="font-title col-sm-1 control-label no-padding-right" 
				       for="queryQuestionTitle">问题名称</label>
				<div class="col-sm-2">
			...
			<div class="modal-body">
				<div class="row">
					<form id="formInput" class="form-horizontal" role="form" >
						<input type="hidden" id="hiddenSurveyQuestionId" name="hiddenSurveyQuestionId" />
						<div class="form-group">
						    <label class="font-title col-sm-2 control-label no-padding-right"
								for="inputSurveyId">调研(*)</label>
							<div class="col-sm-4">
								<select id="inputSurveyId" name="inputSurveyId" class="chosen-select form-control" required >
									<option value=""></option>
									<c:forEach var="item" items="${surveyInfos}">
										<option value="${item.surveyId}">${item.surveyTitle}</option>
									</c:forEach>
								</select>
							</div>
							<label class="font-title col-sm-2 control-label no-padding-right"
								for="inputQuestionId">问题名称(*)</label>
							<div class="col-sm-4">
								<select id="inputQuestionId" name="inputQuestionId" class="chosen-select form-control" required >
									<option value=""></option>
...
<script type="text/javascript">
	...
	//0-预处理客户查询条件(其他页面传入)
	<c:if test="${surveyInfo!=null}">
	   console.log("request surveyId:"+${surveyInfo.surveyId});
	   $("#querySurveyTitle").val("${surveyInfo.surveyTitle}");
	   $("#querySurveyId").val(   "${surveyInfo.surveyId}" );
	</c:if>
	//用户重新输入客户姓名清空queryClientId
	$("#querySurveyTitle").change(function(){
		$("#querySurveyId").val("");
	});
	...
	$(function() {
		...
		dataTableObj = $('#dynamic-table').DataTable({
			...
			"ajax" : {
				url : '../survey/surveyQuestion/',
				type : 'get',
				data : function(req) {
					//1-过滤参数(设置分页参数)
				    filterRequest(req);
				    //2-设置请求参数
				    if($("#querySurveyId").val()!="")
						req.surveyId=$("#querySurveyId").val();
		...
		$('#btnAdd').on('click', function(e) {
			$("#hiddenSurveyQuestionId").val(-1);
			$("#inputSurveyId").val($("#querySurveyId").val());
			$("#inputSurveyId").trigger("chosen:updated");
		    $("#inputQuestionId").val("");
		...
		$('#btnReturn').on('click', function(e) {
		    e.preventDefault(); //阻止默认事件Post
		    var strUrl="../survey/surveyInfoPage";
		    showPageContent(strUrl,"调研信息");
		});
	...
	function editObject(rowIndex) {
		...
		$("#hiddenSurveyQuestionId").val(row["surveyQuestionId"]);
		$("#inputSurveyId").val(row["surveyId"]);
		$("#inputSurveyId").trigger("chosen:updated");
	...
	function postObject() {
	    var obj=new Object();
		obj.surveyQuestionId =$("#hiddenSurveyQuestionId").val();
		obj.surveyId  =$("#inputSurveyId").val();
		obj.questionId=$("#inputQuestionId").val();
		var jsonStr = JSON.stringify(obj);
		//console.log(jsonStr);
		var httpMethod="post";
		if($("#hiddenSurveyQuestionId").val()<=0)
		  httpMethod="post";
		else
		  httpMethod="put";
		$.ajax({
	...
	function deleteObject(deleteId) {
		...
		$.ajax({
			type : "delete",
			url : "../survey/surveyQuestion/"+deleteId,
			error : function() {
			    showError('提交失败!');
			},
			success : function(data) {
