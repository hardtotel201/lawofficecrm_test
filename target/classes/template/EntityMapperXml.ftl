...
<mapper namespace="${mapperInterfacePath}.${modelClassName}Mapper">
  ...
  <select id="findAllByRequestObj" parameterType="${modelClassName}" resultType="${modelClassName}">
  ...
  <insert id="insert" parameterType="${modelClassName}" flushCache="true" useGeneratedKeys="true" keyProperty="${indexName}" >
      insert into ${modelClassName}(<#list fields as e><#if e.beanName!=indexName> ${e.beanName}<#if e_has_next>,</#if></#if></#list>)
      values(<#list fields as e><#if e.beanName!=indexName>${'#{'}${e.beanName},jdbcType=${e.myBatisType}${'}'}<#if e_has_next>,</#if></#if></#list>)
  </insert>
  
   <update id="update" parameterType="${modelClassName}" >
     update ${modelClassName} 
     set <#list fields as e><#if e.beanName!=indexName> ${e.beanName}=${'#{'}${e.beanName},jdbcType=${e.myBatisType}${'}'}<#if e_has_next>,</#if> </#if></#list>
